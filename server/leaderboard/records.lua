    ...
    if carClass then
        rows = MySQL.query.await(
            [[SELECT tr.track, tr.car_class, MIN(tr.best_time) AS lap_time_ms,
                     ...
                     (SELECT p.rank FROM track_records tr2 JOIN players p ON p.id = tr2.player_id WHERE tr2.track = tr.track AND tr2.car_class = tr.car_class ORDER BY tr2.best_time ASC LIMIT 1) AS rank_title,
    ...
    if #rows == 0 then
        rows = MySQL.query.await(
            [[SELECT r.track, 'S' AS car_class, MIN(r.best_ms) AS lap_time_ms,
                     ...
                     (SELECT p.rank FROM racelines r2 JOIN players p ON p.id = r2.player_id WHERE r2.track = r.track ORDER BY r2.best_ms ASC LIMIT 1) AS rank_title
              FROM racelines r
              GROUP BY r.track
